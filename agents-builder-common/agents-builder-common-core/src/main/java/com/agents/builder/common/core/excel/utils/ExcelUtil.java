package com.agents.builder.common.core.excel.utils;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.core.io.resource.ClassPathResource;
import cn.hutool.core.util.IdUtil;
import com.agents.builder.common.core.excel.convert.ExcelBigNumberConvert;
import com.agents.builder.common.core.excel.core.*;
...
public class ExcelUtil {
    ...
    public static Map<String,List<Map<String,Object>>> importExcelMultiSheetToMap(InputStream is) {
        ...
        try (ExcelReader excelReader = EasyExcel.read(is).build()) {
            excelReader.excelExecutor().sheetList().forEach(sheet -> {
                ...
                map.put(sheet.getSheetName(),listener.getResult().getList());
            });
    ...
    public static <T> Map<String,List<T>> importExcelMultiSheet(InputStream is, Class<T> clazz) {
        Map<String,List<T>> map = new HashMap<>();
        try (ExcelReader excelReader = EasyExcel.read(is).build()) {
            excelReader.excelExecutor().sheetList().forEach(sheet -> {
                DefaultExcelListener listener = new DefaultExcelListener<>(false);
                ReadSheet readSheet = EasyExcel.readSheet(sheet.getSheetNo()).head(clazz).registerReadListener(listener).build();
                excelReader.read(readSheet);
                map.put(sheet.getSheetName(),listener.getExcelResult().getList());
            });
