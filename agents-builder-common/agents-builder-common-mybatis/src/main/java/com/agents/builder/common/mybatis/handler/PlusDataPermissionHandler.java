...
import com.agents.builder.common.mybatis.enums.DataScopeType;
...
public class PlusDataPermissionHandler {
    ...
    public Expression getSqlSegment(Expression where, String mappedStatementId, boolean isSelect) {
        ...
        if (ObjectUtil.isNull(currentUser)) {
            ...
            if (ObjectUtil.isNull(currentUser))return where;
            DataPermissionHelper.setVariable("user", currentUser);
            DataPermissionHelper.setVariable("targetType", dataPermissionCacheMap.get(mappedStatementId).targetType().getKey());
        }
        DataPermissionHelper.setVariable("isSelect",isSelect);
    ...
    private LoginUser getCurrentUser() {
        ...
        try {
            ...
            UserContextHolderInterceptor userContextHolderInterceptor = SpringUtils.getBean(UserContextHolderInterceptor.class);
    ...
    private String buildDataFilter(DataColumn[] dataColumns, boolean isSelect) {
        ...
        // 获取角色权限泛型
        DataScopeType type = DataScopeType.findCode(user.getRole());
        if (ObjectUtil.isNull(type)) {
            throw new ServiceException("角色数据范围异常 => " + user.getRole());
        }
        // 全部数据权限直接返回
